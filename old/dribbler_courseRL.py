...
def build_model(input_size, output_size):
    ...
    # observatino input
    model.add(layers.InputLayer(input_shape=(1, input_size)))
...
def build_agent(model, total_actions):
    ...
    dqn = DDPGAgent(model=model, memory=memory, policy=policy, 
    nb_actions=total_actions, nb_steps_warmup=4000, target_model_update=1e-2)
...
def make_slopes(space, width, height, n, bwidth, goal_right_top, goal_right_bottom, goal_left_top, goal_left_bottom, width_tiang, doGoal):    
    const_val = n+bwidth
...
def player_is_gerak_menjauh(player, ball, threshold_player_vel, threshold_angle, threshold_ok_distance):
    ...
    angle_goal = get_ball_to_target_angle(player, ball.body.position) # trick ball as goal
...
def make_data_masuk_dribble(player, ball, target_position):
    ...
    # pakai absolute biar model bisa punya memory of like mana ada tiang/obstacle, relativ gak bisa tau
...
def game(window, width, height, genomes, config, doRandom, asA):
    ...
    # anti ballz stucc
    make_slopes(space, width, height, 35, 10, grtc, grbc, gltc, glbc, width_tiang, False)
    ...
    ========================
    '''
    player, self_team, opo_team, self_goal, opo_goal = get_player_team_goal(team_A, team_B, None, None, asA)
    ...
    while isRun:
        ...
        if(doVisualize):
            clear_screen(window)
            draw(window, [ball, *team_A, *team_B], score_data, space, draw_options, True)
        ...
        if(distance_ball_target > wp_threshold_disance):
            ...
            target_wp = get_random_wp(ball, ball_to_wp_init_distance, min_x, min_y, max_x, max_y)
            # tambah fitness
            # genomes[0][1].fitness += min_dim + (1/total_iter)*80000
    ...
    ### === END OF WHILE LOOP === ###
    # genomes[0][1].fitness += bonus_touching
    distance_ball_target = calculate_distance(ball.body.position, target_wp)
    distance_player_ball = calculate_distance(player.body.position, ball.body.position) if bonus_touching > 0 else WIDTH
    # kurangi fitness
    # genomes[0][1].fitness -= (distance_ball_target + distance_player_ball)

    genomes[0][1].total_wp_reached = wpke-1
